function  magic(th, algo, loadAll) {
	...
	ctx.drawImage( th, 0, 0, img_size, img_size);

	try {
		data = ctx.getImageData(0, 0, img_size, img_size);
	}
	catch(e) {
	...
	if(loadAll) {
		...
		return _answer.map(c => `rgb(${c.r},${c.g},${c.b})`);
	} else {
		let rgb = algo( data );
		let colorString = String("rgb(" + rgb.r + ", " + rgb.g + ", " + rgb.b + ")");
