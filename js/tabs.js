...
function addTab(url, title) {
	...
	if(ffb.returnOpt()) {
		...
		if(_d.style=='minimalizm') {
			getFavicon(newTab.url, function(data) {
				console.log(data)
				newTab.favicon = data;
				addTabInDB(newTab);
			}, 1); //2
...
var cat = {
	...
	init: function () {
		...
		this.render();
		console.log(global_opt)

		if(global_opt.show_plus_btn != undefined && global_opt.show_plus_btn == false) {
...
var tabs = {
	...
	render: function (obj) {
		...
		if(global_opt.show_plus_btn === undefined || global_opt.show_plus_btn == true) {
...
function remove_file(fileUrl) {
	...
	directory.fs.root.getFile(fileUrl, {create: false}, function(fileEntry) {
		fileEntry.remove(function() {
			...
			delete localStorage.imagesArray; // очистка хранилища с картинками
		}, errorHandler);
	}, errorHandler);
}
...
var ffb = {
	...
	install: function () {
		$('#add_tab').on('hidden.bs.modal', function (e) {
			chrome.runtime.sendMessage('close_hidden_window');
			ffb.refreshForm();
		});
	...
	showPrev: function () {
		...
		ffb._data.favicons = _.getFavicons(url, _.getSiteAddres(url));
		...
		chrome.runtime.sendMessage({captureUrl: url}, function(data) {
			if(data.error) {
	...
	saveBg: function (colors) {
		...
		console.log(ffb._data)
		$('.tab_bg .btn_tab_bg').each(function (i) {
			let _c = colors[i];
			console.log(_c)
			$(this).css('background-color', colors[i] || '#fff');
		});
	},
...
var _ = {
	...
	getFavicons: function(url, hostName) {
		...
		var favs = [faviconURL(url), hostName+'/favicon.ico' ];
		// var favs = ['chrome://favicon/size/16@2x/'+url, 'chrome://favicon/'+url, hostName+'/favicon.ico' ];
